...
with open('Day 10/day10.txt') as file:
    ...
    for k in dirsToCheck:
        ...
        bestRoute = [p for p,d in directions[k]]

    boundaryPoints = len(bestRoute)+1

    bestRoute = [start,*bestRoute,start] # sandwich with the start
    a = 0
    for (ax,ay),(bx,by) in pairwise(bestRoute):
        a += (ay+by)*(ax-bx)
